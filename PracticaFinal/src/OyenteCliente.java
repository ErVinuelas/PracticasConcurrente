	...
	public void run() {
		try {
			...
			if (m1.getTipo() != TipoMensaje.CONEXION) {
				// Cerramos el canal
				Log.error("Se esperaba mensaje de tipo CONEXION pero se recibio mensaje de tipo "
						+ m1.getTipo().toString(), sc);
				fIn.close();
				sc.close();
				return;
			}
			...
			while (!stop) {
				...
				case PEDIR_LISTA:
					if (m.isACK()) {
						...
						/*
						 * Log.debug("Usuarios:", sc); for (Usuario u : serv.userLst.values()) {
						 * Log.debug("Usuario:\n" + u.toString(),sc); }
						 */
						serv.flujoLst.get(usuario)
								.writeObject(new MensajeSolicListaUsuar(serv.userLst.getTabla(), true));
					}
					break;
				...
				case PREPARADO_CS:
					...
					serv.flujoLst.get(mp.getUser())
							.writeObject(new MensajePreparadoSC(mp.getIP(), mp.getPort(), mp.getFileName()));
				...
				case ACTUALIZAR_LISTA:
					...
					serv.flujoLst.get(usuario)
							.writeObject(new MensajeActualizarListaUsuarios(ma.idCliente, ma.nombreArchivo, true));
					break;
